...
class History(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        with open('256lstm20_losses.pickle', 'wb') as handle:
            pickle.dump(self.losses, handle)
    #save five models to use later to show text gen
    def on_epoch_end(self, epoch, logs={}):
        if epoch % 20 == 0:
            self.model.save("256LSTM20_at_epoch{}.hd5".format(epoch))
...
model = load_model('256LSTM20_at_epoch20.hd5')
...
history = History()
model.fit(x_mod, Y_mod, epochs=80, batch_size=128, callbacks = [history])
...
# plt.show()
plt.savefig('lossplot_256lstm20.png')

with open('256lstm20.pickle', 'wb') as handle:
