...
class History(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        with open('128lstm40_losses.pickle', 'wb') as handle:
            pickle.dump(self.losses, handle)

    #save five models to use later to show text gen
    def on_epoch_end(self, epoch, logs={}):
        if epoch % 20 == 0:
            self.model.save("128LSTM40_at_epoch{}.hd5".format(epoch))
...
model = load_model('128LSTM40_at_epoch0.hd5')
...
history = History()
model.fit(x_mod, Y_mod, epochs=60, batch_size=128, callbacks = [history])
...
# plt.show()
plt.savefig('lossplot_128lstm40.png')

with open('128lstm40.pickle', 'wb') as handle:
